...
exports.getBootcamps = asyncHandler(async (req, res, next) => {
    ...
    // Handle select fields and add onto query
    if (req.query.select) {
      const fields = req.query.select.split(',').join(' ');
      query = query.select(fields);
    }

    // Handle sort fields and add onto query
    if (req.query.sort) {
    ...
    // Pagination
    const page = parseInt(req.query.page, 10) || 1; // convert page into number
    const limit = parseInt(req.query.limit, 10) || 25; // default per page
...
exports.bootcampPhotoUpload = asyncHandler(async (req, res, next) => {
  ...
  file.mv(`${process.env.FILE_UPLOAD_PATH}/${file.name}`, async (err) => {
    ...
    res.status(200).json({
      success: true,
      msg: `Photo uploaded for bootcamp ${req.params.id}`,
      data: file.name
    })
  });
})